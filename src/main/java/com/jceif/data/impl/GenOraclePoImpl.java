...
import com.jceif.data.service.GenOraclePoService;
import com.jceif.data.common.TableUtil;

import java.sql.SQLException;
import java.util.*;

@Repository("genOraclePoService")
public class GenOraclePoImpl implements GenOraclePoService {
	...
	public String geneJavaBean(String tableName, String schemaName)
		...
		// package
		sb.append("package " + TableUtil.OraclePoPath + TableUtil.toClassName(tableName).toLowerCase() + " ;");
		sb.append(LINE);
		...
		// import
		String strImport = importPackage(lstTableColumn,tableName);
		sb.append(strImport);
		sb.append(LINE);

		// 类定义

		sb.append("public class " + TableUtil.toOracleClassName(tableName) + " implements Serializable {");
	 ...
	 * @throws SQLException
	 */
	private String importPackage(List<TableColumn> lstTableColumn,String tableName)
			throws SQLException {
		String strRtn = "";
		...
		for (int i = 0; i < lstTableColumn.size(); i++) {
			...
			String columnTypeJava = TableUtil.getJavaType(columnName, dataType,TableUtil.databaseTypeOracle,tableName);
//			String columnTypeJava = TableUtil.typeMap.get(dataType);
	...
	private String defProperty(List<TableColumn> lstTableColumn,String tableName)
		...
		for (int i = 0; i < lstTableColumn.size(); i++) {
			...
			String strJavaType = TableUtil.getJavaType(columnName, dataType,TableUtil.databaseTypeOracle,tableName);
